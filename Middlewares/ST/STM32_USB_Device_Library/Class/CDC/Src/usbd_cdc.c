...
static uint8_t USBD_CDC_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
    ...
    if (pdev->dev_speed == USBD_SPEED_HIGH) {
        ...
        /* ADDED Prepare Out endpoint 3 to receive next packet */
        (void)USBD_LL_PrepareReceive(pdev, FS_LINK_OUT_EP, hcdc->RxBuffer,
                                     CDC_DATA_FS_OUT_PACKET_SIZE);
    }
...
static uint8_t USBD_CDC_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
    ...
    ((USBD_CDC_ItfTypeDef *)pdev->pUserData[pdev->classId])->Receive(hcdc->RxBuffer, &hcdc->RxLength, epnum); // CHANGE
